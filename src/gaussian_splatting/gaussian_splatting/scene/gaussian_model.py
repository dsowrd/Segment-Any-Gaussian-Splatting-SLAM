...
class GaussianModel(nn.Module):
    ...
    def add_from_pcd2_tensor(self, points, colors, rots_, scales_, z_vals_, trackable_idxs):
        ...
        self.new_objects_dc = nn.Parameter(fused_objects.transpose(1, 2).contiguous().requires_grad_(True))
        ...
        self.densification_postfix(self.new_xyz, self.new_features_dc, 
                                   ...
                                   self.new_scaling, self.new_rotation, self.new_objects_dc, self.new_trackable_mask)
        # print("self.new_xyz.shape =", self.new_xyz.shape)
        # print("self.keyframe_idx.shape =", self.keyframe_idx.shape)
        # new_keyframe_idx = torch.zeros((self.new_xyz.shape[0], self.keyframe_idx.shape[1]), device="cuda", dtype=torch.bool)
        new_keyframe_idx = torch.zeros((self.new_xyz.shape[0], 1), device="cuda", dtype=torch.bool)
        # Expanding keyframe_idx table
        # Add new gaussians
        self.keyframe_idx = torch.concat([  self.keyframe_idx.to('cuda'),
    ...
    def cat_tensors_to_optimizer(self, tensors_dict):
        ...
        for group in self.optimizer.param_groups:
            ...
            if stored_state is not None:
                ...
                # print(f"group['params'][0].device = {group['params'][0].device}")
                # print(f"extension_tensor.device = {extension_tensor.device}")

                group["params"][0] = nn.Parameter(torch.cat((group["params"][0].to('cuda'), extension_tensor), dim=0).requires_grad_(True))
                optimizable_tensors[group["name"]] = group["params"][0]

        return optimizable_tensors

    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_objects_dc, new_trackable_mask):
        d = {"xyz": new_xyz,
        ...
        # print(f"self.trackable_mask.device = {self.trackable_mask.device}")
        # print(f"new_trackable_mask.device = {new_trackable_mask.device}")
        self.trackable_mask = torch.concat([self.trackable_mask.to('cuda'), new_trackable_mask], dim=0)
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacity, new_scaling, new_rotation, new_objects_dc, new_trackable_mask)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_objects_dc, new_trackable_mask)
        #torch.cuda.empty_cache()
