...
def getWorld2View_traditional(R, t):
    """
    Given R and t representing the camera's pose in world coordinates (i.e., T_c_w = [R, t]),
    compute the view matrix (i.e., T_w_c = T_c_w^{-1}) using the traditional formula:
    
        V = [R^T, -R^T * t; 0, 1].
    
    Parameters:
      R : a 3x3 torch.Tensor (camera rotation in world)
      t : a 3-element torch.Tensor (camera translation in world)
    
    Returns:
      view : a 4x4 torch.Tensor representing the view matrix.
    """
    # Compute the transpose (which is the inverse of a rotation matrix).
    R_inv = R.t()
    # Compute -R^T * t
...
def getWorld2View_traditional_scaled(R, t, translate=np.array([0.0, 0.0, 0.0]), scale=1.0):
    """
    Given R and t representing the camera's pose in world coordinates 
    (i.e., T_c_w = [R, t]), compute the view matrix (i.e., T_w_c = T_c_w^{-1})
    using the traditional formula with an extra translation offset and scaling 
    applied to the camera center.
    
    This function follows these steps:
      1. Compute the traditional view matrix:
             V = [R^T, -R^T * t; 0, 1]
      2. Invert V to get the camera-to-world transform: C2W = V^{-1}
      3. Adjust the camera center:
             cam_center_new = (cam_center + translate) * scale
      4. Replace the translation part of C2W with cam_center_new and invert back.
    
    Parameters:
      R         : a 3x3 torch.Tensor (camera rotation in world)
      t         : a 3-element torch.Tensor (camera translation in world)
      translate : an array-like of shape (3,) to add to the computed camera center
      scale     : a scalar to multiply the camera center after translation
      
    Returns:
      view : a 4x4 torch.Tensor representing the modified view matrix.
    """
    # Traditional view matrix: V = [R^T, -R^T * t; 0, 1]
    R_inv = R.t()
    t_inv = - torch.mv(R_inv, t)
    view = torch.eye(4, device=R.device)
    view[:3, :3] = R_inv
    view[:3, 3] = t_inv

    # Invert to get camera-to-world matrix:
    C2W = torch.inverse(view)
    # Extract the camera center from C2W (the fourth column, first 3 elements)
    cam_center = C2W[:3, 3]
    
    # Apply additional translation and scaling:
    translate_tensor = torch.tensor(translate, device=R.device, dtype=cam_center.dtype)
    cam_center_new = (cam_center + translate_tensor) * scale
    C2W[:3, 3] = cam_center_new
    
    # Invert again to get the modified view matrix:
    view_modified = torch.inverse(C2W)
    return view_modified
...
class SharedCam(nn.Module):
    def __init__(self, FoVx, FoVy, image, object_mask,
                 cx, cy, fx, fy, original_pose,
        ...
        self.objects = torch.from_numpy(object_mask).long().to('cuda')
        ...
        self.world_view_transform = getWorld2View_traditional(self.R, self.t).transpose(0, 1)
        # self.world_view_transform = getWorld2View_traditional_scaled(self.R, self.t)
    ...
    def setup_cam(self, R, t, rgb_img, object_mask):
        ...
        self.objects[:,:] = torch.from_numpy(object_mask).long()
    ...
    def on_cuda(self):
        ...
        self.objects = self.objects.cuda()

    def setup_obj(self, objects):
        self.objects = objects
        


class MappingCam(nn.Module):
    def __init__(self, cam_idx, R, t, FoVx, FoVy, image,
                 cx, cy, fx, fy,
