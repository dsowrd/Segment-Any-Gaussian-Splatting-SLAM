...
class GaussianSplatting(SLAMParameters):
    def __init__(self):
        ...
        self.saved_scenes = []  # scenes with a flag "trained" and optionally stored images
        ...
        img_sub = message_filters.Subscriber("/rgb_img", Image)
    ...
    def render_saved_scene(self, scene, depth_image, object_mask):
        """
    ...
    def save_scene_images(self):
        ...
        # Prepare common depth image and mask for rendering.
        ...
        for idx, scene in enumerate(self.saved_scenes):
            ...
            rendered_np, rendered = self.render_saved_scene(scene, depth_image, object_mask)
    ...
    def calc_2d_metric(self):
        ...
        for idx, scene in tqdm(enumerate(self.saved_eval_scenes)):
            if idx % 100 == 0:
                # Get the ground truth image.
                if "image" in scene:
                    gt_rgb = scene["image"]
                ...
                # Render the scene after termination.
                rendered_np, ours_rgb_ = self.render_saved_scene(scene, depth_image, object_mask)
        ...
        while True:
            ...
            for scene in self.saved_scenes:
                if not scene.get("trained", False) and all(key in scene for key in ("image", "points", "colors", "pose")):
                    ...
                    # Save the rendered image in the scene dictionary.
                    scene["rendered_image"] = rendered_np
                    scene["trained"] = True
