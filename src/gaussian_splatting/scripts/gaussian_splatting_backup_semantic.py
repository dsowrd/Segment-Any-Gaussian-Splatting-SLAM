...
class GaussianSplatting(SLAMParameters):
    ...
    def synced_callback(self, image_msg, cloud_msg, odom_msg):
        ...
        try:
            ...
            # For segmentation, use a lower resolution for speed.
            seg_scale = 0.5  # For example, half the width and height
            seg_W = int(self.W * seg_scale)
            seg_H = int(self.H * seg_scale)
            seg_image = cv2.resize(cv_image_rgb, (seg_W, seg_H))
            ...
            # outputs.logits has shape [1, num_classes, H_out, W_out].
            # Get the predicted mask (with reduced resolution).
            predicted_mask = outputs.logits.argmax(dim=1).squeeze(0)
            
            # Upscale the predicted mask back to full resolution.
            predicted_mask_np = predicted_mask.cpu().numpy()
            object_mask = cv2.resize(predicted_mask_np.astype(np.uint8),
                                    (self.W, self.H),
        ...
        # Create a SharedCam for rendering.
            ...
            viewpoint_cam.setup_cam(new_rotation, new_translation, synced["image"], synced["object_mask"])
    ...
    def run(self):
        ...
        while True:
            ...
            if self.mapping_new_cams:
                ...
                rr.log("gt_obj_mask", rr.Image(gt_obj.cpu().numpy().astype(np.uint8)))
